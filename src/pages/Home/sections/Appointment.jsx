    ...
    <section className="relative overflow-hidden bg-[#f6f9fe] py-14" id="appointment">
      ...
      <div className="container relative">
        ...
        <div className="grid grid-cols-[.92fr_1.08fr] gap-8 max-lg:grid-cols-1">
          ...
          <form
            className="rounded-2xl border border-[#e3eaf5] bg-white p-7 shadow-[0_18px_45px_rgba(8,29,82,0.1)] max-sm:p-5"
          ...
          >
            ...
            {status === 'success' ? (
              <div className="flex flex-col items-center gap-3 py-10 text-center">
                ...
                <p className="text-lg font-black text-[#07133f]">Appointment Request Sent!</p>
                <p className="text-sm text-[#5a6a8a]">We'll call you back shortly to confirm your booking.</p>
              </div>
            ) : (
              <div className="grid gap-4">
                <label className="grid gap-2 text-xs font-black text-[#142452]">
                  <span>Your Name</span>
                  <input
                    className="h-12 rounded-xl border border-[#dfe7f3] px-4 text-[14px] text-[#07133f] outline-none transition focus:border-[#ff2638] focus:ring-2 focus:ring-[#ff2638]/10 bg-[#f8faff]"
                ...
                <label className="grid gap-2 text-xs font-black text-[#142452]">
                  ...
                  <SelectDropdown options={TREATMENT_OPTIONS} value={form.treatment} onChange={(v) => setForm({ ...form, treatment: v })} variant="light" />
                </label>
                <label className="grid gap-2 text-xs font-black text-[#142452]">
                  <span>Preferred Date</span>
                  <DatePicker value={form.date} onChange={(v) => setForm({ ...form, date: v })} variant="light" />
                </label>
                {status === 'error' && (
                  <p className="text-sm font-semibold text-red-500">Something went wrong. Please try again or call us directly.</p>
                )}
                <button className="primary-btn mt-2 min-h-[52px] w-full justify-center" type="submit" disabled={status === 'sending'}>
